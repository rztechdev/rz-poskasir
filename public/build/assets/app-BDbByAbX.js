...
.container {
 ...
 `).join(""),a=e.map((d,u)=>{const m=d.is_without_payment||d.status==="rejected"&&d.rejection_reason==="Tanpa Pembayaran";return`
 ...
 <td style="text-align: right; border: 1px solid #000; padding: 5px 6px; font-weight: bold;">${m?"-":I(d.total_amount)}</td>
 <td style="text-align: center; border: 1px solid #000; padding: 5px 6px; font-weight: bold; font-size: 11px;">${m?"TANPA PEMBAYARAN":d.status.toUpperCase()}</td>
 </tr>
...
.footer-note {
 ...
 `;this.printDocument(c)},printCabangReport(t){const e=this.stores.find(w=>w.id==t)||this.userStores.find(w=>w.id==t)||{name:"Cabang",booth_number:"-"},n=this.getActiveEvent()||{name:"Event Bazaar UMKM"},r=this.transactions.filter(w=>w.store_id==t),o=r.filter(w=>w.status==="paid"),i=o.reduce((w,g)=>w+g.total_amount,0),s=o.reduce((w,g)=>w+(g.revenue_split?.owner_share||g.total_amount*.75),0);o.reduce((w,g)=>w+(g.revenue_split?.admin_net_share||g.total_amount*.225),0),o.reduce((w,g)=>w+(g.revenue_split?.superadmin_share||g.total_amount*.025),0);const a=o.length,l=o.filter(w=>w.payment_method==="cash").length,c=o.filter(w=>w.payment_method==="qris").length,d=new Date().toLocaleDateString("id-ID",{weekday:"long",day:"numeric",month:"long",year:"numeric",hour:"2-digit",minute:"2-digit",timeZone:Ot}),u=r.map((w,g)=>{const y=w.is_without_payment||w.status==="rejected"&&w.rejection_reason==="Tanpa Pembayaran";return`
 <tr style="${w.status==="cancelled"?"text-decoration: line-through; color: #555;":""}">
 <td style="text-align: center; border: 1px solid #000; padding: 5px 6px;">${g+1}</td>
 ...
 `}).join(""),m=window.__LOGO_BASE64__||window.__LOGO_URL__||"",h=`
 <!DOCTYPE html>
 <html lang="id">
 <head>
 <meta charset="UTF-8">
...
.footer-note {
 ...
 ${m?`
 <td style="width: 65px; border: none; text-align: left; vertical-align: middle; padding: 0;">
 <img src="${m}" style="height: 50px; width: auto; object-fit: contain;">
 </td>
 `:""}
 <td style="border: none; text-align: ${m?"left":"center"}; vertical-align: middle; padding: 0 0 0 10px;">
...
.footer-note { font-size: 8.5pt; text-align: center; color: #555; border-top: 1pt dashed #000; padding-top: 6pt; margin-top: 16pt; }
 ...
 `).join(""),l=e.map((u,m)=>{const h=u.is_without_payment||u.status==="rejected"&&u.rejection_reason==="Tanpa Pembayaran";return`
 <tr style="${u.status==="cancelled"?"text-decoration: line-through; color: #777;":""}">
 <td style="text-align: center; border: 1pt solid #000; padding: 4pt 5pt;">${m+1}</td>
...
.footer-note { font-size: 8.5pt; text-align: center; color: #555; border-top: 1pt dashed #000; padding-top: 6pt; margin-top: 16pt; }
 ...
 `).join(""),l=e.map((d,u)=>{const m=d.is_without_payment||d.status==="rejected"&&d.rejection_reason==="Tanpa Pembayaran";return`
 <tr>
 <td style="text-align: center; border: 1px solid #cbd5e1;">${u+1}</td>
 ...
 <td style="text-align: right; border: 1px solid #cbd5e1; mso-number-format:'\\#\\,\\#\\#0'; ">${m?0:Mt(d.total_amount)}</td>
 <td style="text-align: center; border: 1px solid #cbd5e1; font-weight: bold;">${m?"TANPA PEMBAYARAN":d.status.toUpperCase()}</td>
 </tr>
 `}).join("");const c=`
 <html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">
 <head>
 ...
 <td style="text-align: right; border: 1pt solid #000; padding: 4pt 5pt; font-weight: bold;">${m?"-":I(d.total_amount)}</td>
 <td style="text-align: center; border: 1pt solid #000; padding: 4pt 5pt; font-weight: bold;">${m?"TANPA PEMBAYARAN":d.status.toUpperCase()}</td>
 </tr>
 `}).join(""),c=`
 <html xmlns:o='urn:schemas-microsoft-com:office:office' xmlns:w='urn:schemas-microsoft-com:office:word' xmlns='http://www.w3.org/TR/REC-html40'>
 <head>
